...
async def async_setup_platform():
    ...
    #async with session.get(`http://{}:{}/getSystemData`.format(CONF_HOST,CONF_PORT)):
    async with session.get('http://ba.id.au/getSystemData.json') as resp:
        json = (await resp.json())['aircons']
        print(json)
        for _, acx in enumerate(json):
            print(acx)
            for _, zx in enumerate(json[acx]['zones']):
                print(acx, zx) 
acx = "ac1"
print({acx:{"info":{"state":"off"}}})
asyncio.run(async_setup_platform())